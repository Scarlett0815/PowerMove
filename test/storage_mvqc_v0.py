...
def mvqc(gates, Row, n, storage_flag):
    print("number of gates", len(gates))
    print(gates)
    result_scheduling = gate_scheduling(n, gates)

    print(result_scheduling)

    list_gates = []
    for gates_i in result_scheduling:
        tmp = [gates[i] for i in gates_i]
    ...
    list_gates_copy = list_gates.copy()
    for i in range(Row):
        storage_occ[i] = -2
    lg = []
    for i in range(len(list_gates)):
        if i == 0:
            ...
            lg.append(target_mg)
            list_gates.remove(target_mg)   

    print("if lg == list gates", lg == list_gates_copy)
    ...
    for mg in lg:
        ...
        if storage_flag:  
            ...
            move_in_qubits_copy = move_in_qubits.copy()
            for q in move_in_qubits_copy:
                # pos_q = pos[q]
                # pos_x = pos_q[0]
                # pos_y = pos_q[1]
                # pos_find_flag = False
        ...
        for ms in parallel_move_groups:
            ...
            for i in range(n):
                if i not in list_active_qubits and i in qubits_not_in_storage:
    ...
    return sum(list_movement_duration), cir_fidelity, cir_fidelity_1q_gate, cir_fidelity_2q_gate, cir_fidelity_2q_gate_for_idle, cir_fidelity_atom_transfer, cir_fidelity_coherence